...
<div id="J_crumbsBar" class="crumbs-bar">
	...
	<div class="crumbs-nav">
		<div class="crumbs-nav-main clearfix">
			...
			<c:if test="${fn:length(filterQuery) != 0 }">
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${filterQuery }" var="fq">
						<a title="${fq.value }" onclick="clearFq('${fq.key }')" href="javascript:;" class="crumb-select-item">
							<b>${fq.key }：</b><em>${fq.value }</em><i></i>
						</a>
					</c:forEach>
			</div>
			</c:if>
